...
void SDMenu::Draw(void) const {
	...
	Gui::DrawString(0, 2, 0.8f, WHITE, "D7-Menu -> SDMenu");
...
void SDMenu::Logic(u32 hDown, u32 hHeld, touchPosition touch) {
	...
	if (hDown & KEY_DOWN) {
		if (this->Selection < (int)this->mainButtons.size() - maxTitles+1)	this->Selection+= 5;
	}
        if (hDown & KEY_RIGHT) {
		if (this->Selection < (int)this->mainButtons.size() - maxTitles+1)	this->Selection++;
	}
        if (hDown & KEY_LEFT) {
	...
	if (hDown & KEY_R) {
		if (this->Selection < (int)this->mainButtons.size() -4)	this->Selection+= 15;
	}
