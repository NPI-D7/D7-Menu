...
void MainMenu::Draw(void) const {
  ...
  RenderD7::Draw::TextCentered(0, 2, 0.8f, COOLWHITE, "D7-Menu", 400);
  D7TM::CardLoop();
  RenderD7::Draw::Text(2, 50, 0.7f, RenderD7::Color::Hex("#111111"),
                       "State: " + CardStatus);
  ...
  for (int i = 0; i < (int)this->MMButtons.size(); i++) {
    if (this->Selection == i) {
      RenderD7::Draw::Rect(this->MMButtons[i].x - 2, this->MMButtons[i].y - 2,
                           this->MMButtons[i].w + 4, this->MMButtons[i].h + 4,
                           BUTTONSELECTCOLOR);
      RenderD7::Draw::Rect(this->MMButtons[i].x, this->MMButtons[i].y,
                           this->MMButtons[i].w, this->MMButtons[i].h,
                           BUTTONUNSELECTCOLOR);
    } else {
      RenderD7::Draw::Rect(this->MMButtons[i].x, this->MMButtons[i].y,
                           this->MMButtons[i].w, this->MMButtons[i].h, BUTTON);
    }
  }
  RenderD7::Draw::TextCentered(0, MMButtons[0].y + 10, 0.6f, BLACK, "SDCard");
  RenderD7::Draw::TextCentered(0, MMButtons[1].y + 10, 0.6f, BLACK, "RenderD7");
  RenderD7::Draw::TextCentered(0, MMButtons[2].y + 10, 0.6f, BLACK, "Exit");
...
void MainMenu::Logic(u32 hDown, u32 hHeld, u32 hUp, touchPosition touch) {
  ...
  if (hHeld & KEY_SELECT) {
    ...
    RenderD7::Draw::Text(1, 90, 0.8f, BLACK, "Press \uE045 to exit!");
    RenderD7::OnScreen(Bottom);
  }
  ...
  if (hDown & KEY_DOWN) {
    if (this->Selection < (int)this->MMButtons.size() - 1)
      this->Selection++;
  }
